...
function sessionsDirNameFor(cwd) {
	return `--${(cwd.startsWith("/") ? cwd.slice(1) : cwd).replaceAll("/", "-")}--`;
}
/**
...
async function readSessionLog(artifactPath) {
	...
	for (const event of events) {
		if (SURFACE_ELIGIBLE_TYPES.has(event.type) && typeof event.surfaceOp !== "string") event.surfaceOp = "append";
		if (!Array.isArray(event.sourceEventSeqs)) continue;
		const selfSeq = event.seq;
		const remapped = event.sourceEventSeqs.map((s) => typeof s === "number" && oldToNew.has(s) ? oldToNew.get(s) : NaN).filter((s) => Number.isInteger(s) && s >= 0 && s < selfSeq);
		if (event.type === "assistant/message") event.sourceEventSeqs = remapped;
		else if (remapped.length > 0) event.sourceEventSeqs = remapped;
	...
	return ctx.storageDomain.open(trashDomainSpec).then((trash) => {
			...
			handler: async (req, res) => {
				...
				try {
					await withMutationLock(async () => {
						...
						try {
							const targetDir = join(sessionsRoot, sessionsDirNameFor(realTarget), id);
							...
							let split = -1;
							for (let offset = 4; offset + 4 <= raw.length; offset++) if (raw[offset] === magic[0] && raw[offset + 1] === magic[1] && raw[offset + 2] === magic[2] && raw[offset + 3] === magic[3]) {
								split = offset;
								break;
							}
							...
							const headerLine = new TextDecoder().decode(decompress(new Uint8Array(headerFrame))).trim();
							const header = JSON.parse(headerLine);
							header.cwd = realTarget;
							const rewrittenHeader = zlib.zstdCompressSync(Buffer.from(`${JSON.stringify(header)}\n`, "utf8"), { params: { [zlib.constants.ZSTD_c_checksumFlag]: 1 } });
							await writeFile(zstdPath, Buffer.concat([rewrittenHeader, eventsFrame]));
							const verifyText = new TextDecoder().decode(decompress(new Uint8Array(await readFile(zstdPath))));
							if (JSON.parse(verifyText.slice(0, verifyText.indexOf("\n"))).cwd !== realTarget) throw new Error("header rewrite verification failed");
							try {
								await (await ctx.workspaceRegistry.resolveByPath(fromCwd))?.detachSession(id);
							} catch (detachError) {
								ctx.logger.warn(`[dsh-session-manager] move ${id}: detaching from source workspace failed:`, detachError);
							}
							try {
								await (await ctx.workspaceRegistry.resolveByPath(realTarget))?.attachSession(id);
							} catch (attachError) {
							...
							try {
								await rm(join(sessionsRoot, sessionsDirNameFor(realTarget), id), {
									recursive: true,
									force: true
								});
							} catch {}
		...
		ctx.webServer.register({
			...
			handler: async (req, res) => {
				...
				if (typeof rawPath !== "string" || rawPath.trim().length === 0) return respond(res, 400, {
					ok: false,
					error: "invalid-path"
				});
				try {
					const dir = await realpath(rawPath);
					if (!(await stat(dir)).isDirectory()) return respond(res, 400, {
						ok: false,
					...
					const dirs = [];
					const files = [];
					for (const dirent of dirents) if (dirent.isDirectory()) dirs.push({
						name: dirent.name,
						type: "directory"
					});
					else files.push({
					...
					dirs.sort(byName);
					files.sort(byName);
					const capped = [...dirs, ...files].slice(0, DIR_LIST_CAP);
