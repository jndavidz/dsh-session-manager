window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		const MOVE_ROUTE = "/dsh-session-manager/move";
		/** Browse one host directory so the panel can hand out `@path` references. */
		const LIST_DIR_ROUTE = "/dsh-session-manager/list-dir";
		//#endregion
...
`;
		...
		*/
		function sessionMention(sessionId, label) {
			...
			return `@[${label.replace(/[[\]()]/g, " ").replace(/\s+/g, " ").trim() || shortSessionId(sessionId)}](dsh-session:${payload})`;
		}
		...
		* back to execCommand immediately — the button feedback must feel instant. */
		function copyText(text) {
			...
			const modern = window.navigator.clipboard?.writeText(text).then(() => true, () => legacy());
			if (modern === void 0) return Promise.resolve(legacy());
			return Promise.race([modern, new Promise((resolve) => window.setTimeout(() => resolve(legacy()), 600))]);
		}
		...
		function SessionManager({ useSessions, useWorkspaces, api, sessions, workspaceActions, close }) {
			...
			const [copiedMid, setCopiedMid] = (0, react.useState)(null);
			...
			const renderWorkspaceLabel = (group, index, expanded) => {
			...
			const renderWorkspaceGroup = (group, index) => {
				...
				}, renderWorkspaceLabel(group, index, expanded), expanded && (0, react.createElement)("ul", { className: "dsh-delete-session__list" }, ...group.rows.map((session) => renderRow(session, false))));
			};
			...
			const renderRow = (session, isArchived) => {
				...
				return (0, react.createElement)("li", {
				...
				}, (0, react.createElement)("span", { className: "dsh-delete-session__row-title-text" }, session.displayTitle || session.id), (0, react.createElement)("span", {
					className: "dsh-sm__sid",
					title: `${session.id}\n${strings.copyId}`,
					onClick: (event) => {
						const el = event.currentTarget;
						event.stopPropagation();
						copyText(session.id).then((ok) => {
							if (!ok) return;
					...
					onClick: () => {
						copyText(sessionMention(session.id, session.displayTitle || session.id)).then((ok) => {
							if (!ok) return;
		...
		*/
		function FileBrowserSection({ workspaces }) {
			const strings = useLocaleStrings();
			const [open, setOpen] = (0, react.useState)(false);
			const [wsPath, setWsPath] = (0, react.useState)("");
			const [dir, setDir] = (0, react.useState)("");
			const [entries, setEntries] = (0, react.useState)([]);
			const [busy, setBusy] = (0, react.useState)(false);
			const [error, setError] = (0, react.useState)(null);
			const [copiedFile, setCopiedFile] = (0, react.useState)(null);
			const loadDir = (0, react.useCallback)(async (target) => {
				if (target === "") return;
				setBusy(true);
				try {
					...
					const data = await response.json().catch(() => ({}));
					if (!response.ok || data.ok !== true || data.entries === void 0) {
			...
			const toggle = (0, react.useCallback)(() => {
				setOpen((value) => !value);
				if (!open && dir === "") {
					const preferred = workspaces.some((workspace) => workspace.path === "/mnt/d/repos") ? "/mnt/d/repos" : workspaces.find((workspace) => typeof workspace.path === "string" && workspace.path !== "")?.path;
			...
			return (0, react.createElement)("div", { className: "dsh-delete-session__group" }, (0, react.createElement)("button", {
				type: "button",
				className: "dsh-delete-session__group-toggle",
				onClick: toggle,
				"aria-expanded": open || void 0
			}, (0, react.createElement)("span", { className: "dsh-delete-session__group-toggle-label" }, strings.filesGroup), (0, react.createElement)("span", { className: "dsh-delete-session__group-toggle-chevron" }, open ? strings.collapse : strings.expand)), open && (0, react.createElement)("div", { className: "dsh-sm__files" }, workspaces.length > 0 && (0, react.createElement)("div", { className: "dsh-sm__files-bar" }, (0, react.createElement)("select", {
			...
			}, workspaces.map((workspace) => (0, react.createElement)("option", {
				key: workspace.path ?? workspace.title ?? "",
				value: workspace.path ?? ""
			}, workspace.title ?? workspace.path ?? ""))), (0, react.createElement)("button", {
				type: "button",
			...
			})), (0, react.createElement)("div", { className: "dsh-sm__files-path" }, dir === "" ? "—" : dir), error !== null && (0, react.createElement)("div", { className: "dsh-delete-session__notice dsh-delete-session__notice--error" }, `${strings.filesLoadFailed} (${error})`), error === null && !busy && entries.length === 0 ? (0, react.createElement)("div", { className: "dsh-delete-session__empty" }, strings.filesEmpty) : null, (0, react.createElement)("div", { className: "dsh-sm__files-list" }, entries.map((entry) => {
				const full = joinDir(dir, entry.name);
				const copied = copiedFile === full;
				return (0, react.createElement)("div", {
					key: `${entry.type}:${entry.name}`,
					className: "dsh-sm__file-row"
				}, (0, react.createElement)("span", { className: "dsh-sm__file-icon" }, entry.type === "directory" ? "📁" : "📄"), entry.type === "directory" ? (0, react.createElement)("span", {
					className: "dsh-sm__file-name dsh-sm__file-name--dir",
					title: full,
					onClick: () => void loadDir(full)
				}, entry.name) : (0, react.createElement)("span", {
					className: "dsh-sm__file-name",
					title: full
				}, entry.name, typeof entry.size === "number" ? (0, react.createElement)("span", { className: "dsh-sm__file-size" }, ` ${formatBytes(entry.size)}`) : null), (0, react.createElement)("button", {
					type: "button",
			...
			}))), (0, react.createElement)("div", { className: "dsh-delete-session__group-hint" }, strings.filesHint));
		}
		...
		function SessionDrawer({ api, sessions }) {
			...
			const [morePos, setMorePos] = (0, react.useState)(null);
			const [newestFirst, setNewestFirst] = (0, react.useState)(true);
			const [copiedSid, setCopiedSid] = (0, react.useState)(null);
			const [copiedMid, setCopiedMid] = (0, react.useState)(null);
			const [dragWorkspaceId, setDragWorkspaceId] = (0, react.useState)(null);
			const [dropSlot, setDropSlot] = (0, react.useState)(null);
			const dropSlotRef = (0, react.useRef)(null);
			...
			const handleMove = (0, react.useCallback)(async (sessionId, targetCwd) => {
				setBusyId(sessionId);
				try {
					...
					const code = error instanceof Error ? error.message : "";
					const friendly = code === "move-partial" ? strings.movePartial : code === "same-workspace" ? strings.moveCurrent : "";
					const suffix = friendly !== "" ? ` (${friendly})` : code !== "" ? ` (${code})` : "";
			...
			const renderRow = (row) => {
				...
				return (0, react.createElement)("li", {
				...
				}, (0, react.createElement)("span", { className: "dsh-delete-session__row-title-text" }, row.title || row.sessionId), (0, react.createElement)("span", {
					className: "dsh-sm__sid" + (copiedSid === row.sessionId ? " dsh-sm__sid--copied" : ""),
					title: `${row.sessionId}\n${strings.copyId}`,
					onClick: (event) => {
						event.stopPropagation();
						copyText(row.sessionId).then((ok) => {
							if (!ok) return;
					...
					onClick: () => {
						copyText(sessionMention(row.sessionId, row.title || row.sessionId)).then((ok) => {
							if (!ok) return;
				...
				}), moreOpenId === row.sessionId && morePos !== null && (0, react_dom.createPortal)((0, react.createElement)("div", {
				...
				}, strings.fork), (0, react.createElement)("div", { className: "dsh-delete-session__more-sep" }), (0, react.createElement)("div", { className: "dsh-delete-session__more-heading" }, strings.moveSection), ...workspaces.length > 0 ? workspaces.map((workspace) => (0, react.createElement)("button", {
					key: workspace.path,
					type: "button",
					className: "dsh-delete-session__more-item",
					...
					onClick: () => {
						setMoreOpenId(null);
						handleMove(row.sessionId, workspace.path);
					}
				}, `${workspace.title}${row.cwd === workspace.path ? ` ${strings.moveCurrent}` : ""}`)) : [(0, react.createElement)("div", {
					key: "move-empty",
					className: "dsh-delete-session__more-heading"
				}, strings.moveFailed)], (0, react.createElement)("button", {
					type: "button",
				...
				}, strings.delete)), document.body)), row.archived && (0, react.createElement)(_deepseek_ai_dsh_client_ui_primitives.Button, {
					className: "dsh-row-action",
					variant: "outline",
					size: "sm",
					disabled: busy,
			...
			const renderWorkspaceGroup = (group, index) => {
				...
				return (0, react.createElement)("div", {
					...
					"data-line-top": dropSlot === `before:${group.workspaceId}` || void 0,
					"data-line-end": dropSlot === "__end__" && next === null || void 0
				}, renderWorkspaceLabel(group, index, expanded), expanded && (0, react.createElement)("ul", { className: "dsh-delete-session__list" }, ...group.rows.map((row) => renderRow(row))));
			};
...
/* [nav-bubble 本地验证补丁] settings nav 齿轮 -> 会话气泡 (Lucide message-circle) */
;(function () {
  var MARKER = "data-dsh-session-manager-nav"
  var mask = "%3Csvgxmlns=%27http://www.w3.org/2000/svg%27width=%2724%27height=%2724%27viewBox=%27002424%27fill=%27none%27stroke=%27black%27stroke-width=%272%27stroke-linecap=%27round%27stroke-linejoin=%27round%27%3E%3Cpathd=%27M2.99216.342a22001.0941.167l-1.0653.29a110001.2361.168l3.413-.998a220011.099.0921010010-4.777-4.719%27/%3E%3C/svg%3E"
  var css = "[data-dsh-session-manager-nav] > svg:first-child{display:none}" +
    "[data-dsh-session-manager-nav]::before{content:\"\";flex:none;width:16px;height:16px;background:currentColor;" +
  ...
  style.id = "dsh-delete-session-nav-icon"
  style.textContent = css
  document.head.append(style)
  var sync = function () {
    for (var b of document.querySelectorAll("[role=\"dialog\"] nav button")) {
      var label = (b.textContent || "").trim()
      var mm = label === "会话管理" || label === "Session Manager"
...
/* [nav-bubble] settings nav 齿轮 -> 会话气泡 (Lucide message-circle) (Lucide, 与 dsh-memory/dsh-achievements 同源) */
;(function () {
  var MARKER = "dsh-session-manager-nav"
  var mask = "%3Csvg%20xmlns%3D%27http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%27%20width%3D%2724%27%20height%3D%2724%27%20viewBox%3D%270%200%2024%2024%27%20fill%3D%27none%27%20stroke%3D%27black%27%20stroke-width%3D%272%27%20stroke-linecap%3D%27round%27%20stroke-linejoin%3D%27round%27%3E%3Cpath%20d%3D%27M2.992%2016.342a2%202%200%200%201%20.094%201.167l-1.065%203.29a1%201%200%200%200%201.236%201.168l3.413-.998a2%202%200%200%201%201.099.092%2010%2010%200%201%200-4.777-4.719%27%2F%3E%3C%2Fsvg%3E"
  ...
  var sync = function () {
    for (var b of document.querySelectorAll("[role=\"dialog\"] nav button")) {
      var label = (b.textContent || "").trim()
      var mm = label === "会话管理" || label === "Session Manager" || (label.indexOf("Theme") !== -1)
      if (mm) b.setAttribute(MARKER, "")
      else b.removeAttribute(MARKER)
    }
  }
