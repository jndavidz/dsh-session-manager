...
- **Move session**: the row "More" menu lists every workspace — pick one and the session is re-created under that cwd via the official persistence channel (fresh id), the original row is archived away, and the original artifact lands in the trash for undo
